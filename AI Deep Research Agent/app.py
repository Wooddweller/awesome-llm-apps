...
# ---------- Simple HTML parser to extract Executive Summary ----------
# class ExecSummaryParser(HTMLParser):
#     def __init__(self):
#         super().__init__()
#         self.in_exec_section = False
...
#     def handle_starttag(self, tag, attrs):
#         # Look for <h1> or <h2> with text containing "Executive"
#         if tag in ("h1", "h2"):
...
#             attrs_str = " ".join(f'{k}="{v}"' for k, v in attrs)
#             if attrs_str:
#                 self.buffer.append(f"<{tag} {attrs_str}>")
#             else:
#                 self.buffer.append(f"<{tag}>")
...
#         if self.in_exec_section:
#             self.buffer.append(f"</{tag}>")
#             # Stop when first major section ends (rough heuristic)
...
#         # Detect heading that mentions "Executive" (e.g., "Executive Summary")
#         if getattr(self, "current_tag", None) in ("h1", "h2") and "Executive Summary / Introduction" in text.lower():
#             self.in_exec_section = True
#         if self.in_exec_section:
#             self.buffer.append(text)
...
if st.session_state.state == "input":
    ...
    with col1:
        ...
        #         f.write(html_content)
        #     st.success("Report saved as temp.html (you can open it in a browser).")
    ...
    #     st.markdown(html_content, unsafe_allow_html=True)
        ...
        #     # Optional: delete temp.html
        #     temp_path = os.path.join(os.path.dirname(__file__), REPORT_FILE)
        #     if os.path.exists(temp_path):
        #         os.remove(temp_path)

        #     # Preserve API keys, reset everything else
        #     composio_key = st.session_state.get("composio_key")
        #     openai_key = st.session_state.get("openai_key")
        #     st.session_state.clear()
        #     st.session_state["composio_key"] = composio_key
        #     st.session_state["openai_key"] = openai_key
        #     st.session_state.state = "input"
        #     st.rerun()

    # with col2:
        # st.markdown("### (Optional) Full HTML preview")
        # st.markdown(html_content, unsafe_allow_html=True)
...
# def extract_executive_summary(html: str) -> str:
#     """Return only the Executive Summary/Introduction section from the report HTML."""
#     if not html:
#         return ""
...
#         # Heuristic: look for a heading containing "Executive Summary" or "Introduction"
#         candidates = []
#         for tag in soup.find_all(["h1", "h2", "h3"]):
#             text = (tag.get_text() or "").strip().lower()
#             if "executive summary" in text or "introduction" in text:
#                 candidates.append(tag)

#         if not candidates:
#             return html  # fallback: show full HTML
...
#     httpd = socketserver.TCPServer(("", REPORT_PORT), Handler)
...
# if "report_server" not in st.session_state:
#     st.session_state.report_server = None  # will hold the TCPServer instance
...
#     summary_html = extract_executive_summary(html_content)
#     st.markdown("### Executive Summary")
...
#             # Start new HTTP server + open browser
#             st.session_state.report_server = start_report_server()
#             st.success(
#                 f"Report server running at http://localhost:{REPORT_PORT}/{REPORT_FILE}"
#             )
...
# #             # Start new HTTP server + open browser
# #             st.session_state.report_server = start_report_server()
# #             st.success(f"Report server running at http://localhost:{REPORT_PORT}/{REPORT_FILE}")
...
# 3. Click **Research**.
# 4. When interrupted, **Approve** or **Edit** the questions.
# 5. After completion, click **View report** to open the full HTML report page.
# 6. Use **Restart** to begin a new session.
# """
#     )
